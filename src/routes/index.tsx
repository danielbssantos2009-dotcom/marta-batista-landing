        ...
        <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {pillars.map((p, i) => (
            <Reveal key={p.title} delay={i * 100}>
              <div className="group relative h-full glass rounded-[28px] p-7 transition-all duration-500 hover:-translate-y-2 hover:shadow-float">
                <div
                  className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl text-cream transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3"
                  style={{ background: "var(--gradient-cta)" }}
                >
                  <p.icon className="h-6 w-6" strokeWidth={1.8} />
                </div>
                <h3 className="text-xl font-display text-[color:var(--moss)]">{p.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-[color:var(--moss)]/70">{p.text}</p>
              </div>
            </Reveal>
          ))}
        </div>
